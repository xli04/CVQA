"""
Visual Question Answering Dataset with Memory Replay for Continual Learning

This module provides dataset classes for continual learning in Visual Question Answering (VQA),
supporting memory replay mechanisms to mitigate catastrophic forgetting. The implementation
includes:

1. Base VQA dataset class for standard task-specific training
2. Memory-augmented dataset class for continual learning with exemplar replay
3. Data loading utilities for both task-incremental and domain-incremental scenarios
4. Evaluation metrics for VQA accuracy assessment

The datasets handle loading image features, processing questions, and preparing data
in formats compatible with the MM-Prompt architecture.
"""
...
# Set up path configuration for dataset access
...
class VQAFineTuneDataset(Dataset):
    """
    Dataset for fine-tuning on VQA tasks with support for task-specific categories.
    
    Handles loading image features, tokenizing questions, and preparing data
    for different categories of VQA tasks. Can be used as a standard dataset
    or as part of a continual learning setup.
    """
    def __init__(self, coco_Ours, Examplar_set, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train', task='q_what', cates=[0,1,2]):
        """
        Initialize the VQA fine-tuning dataset.
        
        Args:
            coco_Ours: List of task identifiers
            Examplar_set: Set of exemplar data points for memory replay
            split: Data split to use (train, val, test, etc.)
            raw_dataset: Raw dataset object (optional)
            rank: Process rank for distributed training
            topk: Limit to top-k examples (for debugging)
            verbose: Whether to print detailed information
            args: Arguments containing model and training configuration
            mode: Dataset mode (train, val, test)
            task: Task identifier (e.g., 'q_what')
            cates: List of category IDs to include
        """
        super().__init__()
        ...
        # Define data sources based on the split provided
        self.sources = split.split(',')
        # if self.verbose:
        #     print('Data sources: ', self.sources,'_' + task)

        # Initialize tokenizer based on backbone model type
        if 't5' in self.args.backbone:
            ...
            # Add special tokens for visual features if needed
        ...
        # Initialize answer normalizer for evaluation
        self.answer_normalizer = VQAEvaluator()

        # Load data based on tasks and categories
        self.img_ids_to_source = {}  # Maps image IDs to their source dataset
        data_info_dicts_cate = []    # Will contain data filtered by category
        self.cate_set = set()        # Tracks categories included in dataset
        ...
        for source in self.sources:
            ...
            with open(data_info_path) as f:
                ...
                # Add exemplars for memory replay
                ...
                for _d in _data_info_dicts:
                    ...
                    try:
                        # Only include data from specified categories
                        if QuesId_task_map[question_id] in cates:
                            self.cate_set.add(QuesId_task_map[question_id])
                            data_info_dicts_cate.append(_d)
                            
                            # Map image IDs to their source datasets for feature loading
                        ...
                        continue  # Skip if question ID not in category map
        ...
        # Limit dataset size if topk is specified (for debugging)
        if self.topk > 0:
            data = data[:self.topk]
            if self.verbose:
        ...
        # Setup for image feature loading
        self.n_boxes = args.n_boxes  # Number of object regions per image
        
        # Map data sources to their feature file paths
        self.source_to_h5 = {
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            Dictionary containing all the processed data for a single example:
            - Image features and bounding boxes (if use_vision is True)
            - Tokenized question
            - Answer label
            - Various metadata like question_id, task category, etc.
        """
        out_dict = {}
        ...
        if self.args.use_vision:
            ...
            # Get image category from mapping
            out_dict['img_cate'] = ImgId_cate_map[img_id]

            # Get image source (e.g., train2014, val2014)
            source = self.img_ids_to_source[img_id] # source: val2014

            # Get the H5 file containing image features
            f = self.source_to_h5[source]

            # Load H5 file if it's not already loaded
            ...
            # Load object features for the image
            ...
            # Load and normalize bounding boxes (to 0 ~ 1)
        ...
        # Tokenize the question with VQA-specific prefix
        input_ids = self.tokenizer.encode(f'vqa: {sent}', max_length=20, truncation=True)

        # Store question ID and category for evaluation
        question_id = datum['question_id']
        out_dict['question_id'] = question_id
        out_dict['ques_label'] = QuesId_task_map[str(question_id)] # Question category mapping
        ...
        # Optimal answer flag (used for some evaluation metrics)
        if 'is_topk_optimal' in datum:
            out_dict['is_topk_optimal'] = datum['is_topk_optimal']

        ###### Answer Label Processing ######
        if 'label' in datum:
            label = datum['label']
            out_dict['label'] = label

            # Handle classifier-based VQA (3129 topk answers)
            if self.args.classifier:
                ...
                # Process raw answers directly from annotation
                # Example format:
                ...
                # Tokenize the answer for generation-based training
                ...
                # Process answer using VQA-style soft labels
                # Based on https://github.com/airsplay/lxmert/blob/master/src/pretrain/lxmert_pretrain.py#L191
                
                # Get all answer options and their scores
                answers = []
                scores = []
                ...
                # Handle case with no valid answers
                if score_sum == 0:
                    answer = ''
                    score = 0.
                else:
                    # Sample an answer according to the score distribution
    ...
    def collate_fn(self, batch):
        ...
        batch_entry['cate_labels'] = torch.zeros(cate_labels_.shape[0], 91).scatter_(1, cate_labels_, 1 ) # [bs, 80]

        ques_labels_ = torch.LongTensor(ques_labels).unsqueeze(1)
        # adjustment
        batch_entry['ques_labels'] = torch.zeros(cate_labels_.shape[0], 11).scatter_(1, ques_labels_, 1 ) # [bs, 10]
...
def get_loader_memory(args, task_id, coco_Ours, Examplar_set, _dset, split='karpathy_train', mode='train',
    ...
    for idx, CateGroup in enumerate(Category_splits[task_id]):
        dataset = VQAFineTuneDataset_memory(
            ...
            args=args,
            mode=mode,
            cates=[i for i in range(91)])

        n = len(dataset)
        size1 = n // 5  # First 20%
        
        if idx == 0:
            dataset = Subset(dataset, range(0, size1))
        elif idx == 1:
            dataset = Subset(dataset, range(size1, 2*size1))
        elif idx == 2:
            dataset = Subset(dataset, range(2*size1, 3*size1))
        elif idx == 3:
            dataset = Subset(dataset, range(3*size1, 4*size1))
        elif idx == 4:
            dataset = Subset(dataset, range(4*size1, n))
...
def get_loader_test(args, coco_Ours, Examplar_set, _dset, split='karpathy_train', mode='train',
    ...
    dataset = VQAFineTuneDataset(
        ...
        mode=mode,
        task=task,
        cates=[i for i in range(91)],) # all categories
...
def get_loader(args, task_id, coco_Ours, Examplar_set, _dset, split='karpathy_train', mode='train',
               batch_size=32, workers=4, distributed=False, gpu=0, topk=-1, task='what'):
    ...
    for idx, CateGroup in enumerate(Category_splits[task_id]):
        dataset = VQAFineTuneDataset(
            ...
            mode=mode,
            task=task,
            cates=Category_splits[task_id][CateGroup],)
...
class VQAEvaluator:
    ...
    def evaluate_raw(self, quesid2ans: dict, is_topk_optimal=None):
        ...
        for quesId, resAns in tqdm(quesid2ans.items(), total=len(quesid2ans), ncols=91):
